...
模板 `m16-ics-invite.ics` 用统一占位符：`LHOST` `URL` `USER` `DOMAIN`。发信前用 sed 一次性替换，并生成**每次唯一的 token 路径**：
...
捕获（拿哈希）与中继（直接打另一台）**不能同时跑**——ntlmrelayx 和 Responder 都占用 445 等端口。流程建议：先捕获模式确认触发；确认 SMB 签名关闭后再切中继。一键脚本见 `m16-auth-capture.sh`，手工等价命令：
...
1. **启动接收端**：`bash ~/osep/payloads/infra/../m16-auth-capture.sh`（或 §4.2 手工命令）。确认监听端口已开、日志路径正确。
