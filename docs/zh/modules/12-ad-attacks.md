...
#      黄金票本体命令在 m12-laps-and-trust-notes.md。
# =============================================================================
set -u
...
mode_cross() {
    ...
    info "   判定与完整命令在 m12-laps-and-trust-notes.md；核心三步："
...
场景：M12 场景 49（读 LAPS）、51（RBCD 前置：找可写/可改委派属性的计算机对象）、
      ...
      与 m12-ad-enum-linux.sh 是同一件事的 Linux / Windows 两侧实现。
...
#       53（信任判定 + 域 SID）。配合 m12-laps-and-trust-notes.md 使用。
...
使用：按小节顺序执行；命令里的占位符替换后再粘贴。Linux 侧批量枚举用
      m12-ad-enum-linux.sh -m laps / -m trust，Windows 侧用
...
# 看目标对象 ACL 用 m12-ad-enum-windows.ps1 -Mode ACL
```
...
- `/etc/hosts` 必须同时能解析子域 DC 与根 DC 的 FQDN（Kerberos 不接受 IP），用
  `m12-kerberos-tickets-linux.sh -m hosts` 生成模板。
