...
| # | 场景摘要 | 模块 | 模块文档 |
|---|---|---|---|
| 1 | 网站接受 Word 文件，并自动打开；目标有杀毒软件 | M01 | [01-word-vba-office](/zh/modules/01-word-vba-office) |
| 2 | Word 会自动打开，但 Office 启动 PowerShell 被阻止 | M01 | [01-word-vba-office](/zh/modules/01-word-vba-office) |
| 3 | Word 可以启动 PowerShell，但第二阶段脚本被扫描拦截 | M01 | [01-word-vba-office](/zh/modules/01-word-vba-office) |
| 4 | Word 入口可用，但 PowerShell 动态编译产生的文件被删除 | M01 | [01-word-vba-office](/zh/modules/01-word-vba-office) |
| 5 | Word payload 成功上线，但文档关闭后会话消失 | M01 | [01-word-vba-office](/zh/modules/01-word-vba-office) |
| 6 | 邮件入口存在，但没有可用的 Office 宏入口 | M02 | [02-hta](/zh/modules/02-hta) |
| 7 | HTA 可以触发，但目标同时有 AppLocker、CLM 和 AMSI | M02 | [02-hta](/zh/modules/02-hta) |
| 8 | HTA 下载和执行放在一起失败，分开后能够工作 | M02 | [02-hta](/zh/modules/02-hta) |
| 9 | 邮件附件中的 JScript 会运行，但普通 EXE 受到限制 | M03 | [03-jscript-dotnettojscript](/zh/modules/03-jscript-dotnettojscript) |
| 10 | JScript 能执行简单内容，但复杂脚本被扫描拦截 | M03 | [03-jscript-dotnettojscript](/zh/modules/03-jscript-dotnettojscript) |
| 11 | 用户会打开 ZIP 中的程序，但宏和脚本入口不可用 | M04 | [04-dll-sideloading](/zh/modules/04-dll-sideloading) |
| 12 | DLL 已被加载，但程序立即闪退 | M04 | [04-dll-sideloading](/zh/modules/04-dll-sideloading) |
| 13 | 目标接受日历邀请，但没有宏执行机会 | M16 | [16-ics-calendar](/zh/modules/16-ics-calendar) |
| 14 | 网站允许上传 ASPX，后台是 IIS，目标安装了杀毒软件 | M10 | [10-web-entry-webshell](/zh/modules/10-web-entry-webshell) |
| 15 | 经典 ASP 网站存在 SQL 注入，可以执行系统命令，但下载器被拦 | M10 | [10-web-entry-webshell](/zh/modules/10-web-entry-webshell) |
| 16 | Web 命令注入只接受很短的命令 | M10 | [10-web-entry-webshell](/zh/modules/10-web-entry-webshell) |
| 17 | 目标没有稳定出网能力，下载式第二阶段无法取得 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 18 | 自定义 EXE 一落地就被删除 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 19 | EXE 能保存、能启动，但开始执行内容时被终止 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 20 | 你需要使用托管工具，但其 EXE 文件不能落地运行 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 21 | 普通 EXE 被 AppLocker 拒绝，但特定目录存在允许规则 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 22 | EXE 规则严格，但 DLL 规则和宿主允许条件不同 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 23 | InstallUtil 不可用，但教材中的其他受信任执行宿主可用 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 24 | 普通脚本入口受限，但 XSL 处理路线可用 | M05 | [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi) |
| 25 | 当前已经是本地管理员，但会话没有提升 | M06 | [06-uac-windows-privesc](/zh/modules/06-uac-windows-privesc) |
| 26 | ASPX 或 SQL 会话是服务账户，并具备模拟权限 | M06 | [06-uac-windows-privesc](/zh/modules/06-uac-windows-privesc) |
| 27 | 自动服务提权工具失败，但你仍能修改某个高权限服务 | M06 | [06-uac-windows-privesc](/zh/modules/06-uac-windows-privesc) |
| 28 | 普通用户能通过浏览器联网，自定义 payload 无法直接回连 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 29 | 用户权限会话能回连，提升为 SYSTEM 后却失联 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 30 | 第一阶段回连成功，第二阶段始终没有出现 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 31 | 目标只允许 HTTPS，但 HTTPS 检查影响通信 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 32 | 普通 HTTP(S) 通信不通，但课程实验允许 DNS 通道 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 33 | 目标限制访问目的域名，且实验基础设施支持域前置 | M09 | [09-c2-egress-channels](/zh/modules/09-c2-egress-channels) |
| 34 | 内部网站只接受来自指定网段的访问 | M08 | [08-pivoting-tunneling](/zh/modules/08-pivoting-tunneling) |
| 35 | 代理能连接内网目标，但目标主动认证到不了你的监听端 | M08 | [08-pivoting-tunneling](/zh/modules/08-pivoting-tunneling) |
| 36 | Linux 上传站会执行 ELF，但程序还要通过业务检查 | M13 | [13-linux](/zh/modules/13-linux) |
| 37 | Linux 目标也有杀毒软件，常见 ELF 被检测 | M13 | [13-linux](/zh/modules/13-linux) |
| 38 | Linux 程序从可控位置加载共享库 | M13 | [13-linux](/zh/modules/13-linux) |
| 39 | sudo 只允许一个编辑器或解释器 | M13 | [13-linux](/zh/modules/13-linux) |
| 40 | 你能覆盖制品，但不能直接登录下载制品的机器 | M13 | [13-linux](/zh/modules/13-linux) |
| 41 | 只有受限 Kiosk 桌面，没有终端 | M14 | [14-kiosk-jea-jit](/zh/modules/14-kiosk-jea-jit) |
| 42 | JEA 会话只暴露少量命令，其中包含权限过宽的文件复制 | M14 | [14-kiosk-jea-jit](/zh/modules/14-kiosk-jea-jit) |
| 43 | JIT 临时管理员权限获批，但有效时间很短 | M14 | [14-kiosk-jea-jit](/zh/modules/14-kiosk-jea-jit) |
| 44 | SQL 账户能登录，但不能运行系统命令 | M11 | [11-mssql](/zh/modules/11-mssql) |
| 45 | Linked Server 能查询，但远程执行失败 | M11 | [11-mssql](/zh/modules/11-mssql) |
| 46 | 凭据工具被拦，且 LSASS 本身受到保护 | M07 | [07-credentials-lsass](/zh/modules/07-credentials-lsass) |
| 47 | Linux 上已有域票据，但你需要访问 Windows 服务 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 48 | 没有 SSH 密码，但存在已认证的复用连接 | M13 | [13-linux](/zh/modules/13-linux) |
| 49 | 没有本地提权路径，但当前域用户能读取 LAPS | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 50 | 控制了非约束委派机器，但还没有域级身份 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 51 | 对计算机对象有相关写权限，但不能直接管理目标主机 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 52 | 控制了服务账户，存在约束委派，但只能访问指定服务 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 53 | 掌握了子域高权限，最终目标在林根 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 54 | 低权限域用户可以申请错误配置的证书模板 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 55 | 没有可用的 ESC1 模板，但 CA 存在可中继的 HTTP 注册入口 | M12 | [12-ad-attacks](/zh/modules/12-ad-attacks) |
| 56 | 凭据有效，但目标只开放 WinRM | M15 | [15-winrm-lateral](/zh/modules/15-winrm-lateral) |
...
- 归属模块：`M01` → [01-word-vba-office](/zh/modules/01-word-vba-office)
...
- 归属模块：`M02` → [02-hta](/zh/modules/02-hta)
...
- 归属模块：`M03` → [03-jscript-dotnettojscript](/zh/modules/03-jscript-dotnettojscript)
...
- 归属模块：`M04` → [04-dll-sideloading](/zh/modules/04-dll-sideloading)
...
- 归属模块：`M16` → [16-ics-calendar](/zh/modules/16-ics-calendar)
...
- 归属模块：`M10` → [10-web-entry-webshell](/zh/modules/10-web-entry-webshell)
...
- 归属模块：`M09` → [09-c2-egress-channels](/zh/modules/09-c2-egress-channels)
...
- 归属模块：`M05` → [05-applocker-clm-amsi](/zh/modules/05-applocker-clm-amsi)
...
- 归属模块：`M06` → [06-uac-windows-privesc](/zh/modules/06-uac-windows-privesc)
...
- 归属模块：`M08` → [08-pivoting-tunneling](/zh/modules/08-pivoting-tunneling)
...
- 归属模块：`M13` → [13-linux](/zh/modules/13-linux)
...
- 归属模块：`M14` → [14-kiosk-jea-jit](/zh/modules/14-kiosk-jea-jit)
...
- 归属模块：`M11` → [11-mssql](/zh/modules/11-mssql)
...
- 归属模块：`M07` → [07-credentials-lsass](/zh/modules/07-credentials-lsass)
...
- 归属模块：`M12` → [12-ad-attacks](/zh/modules/12-ad-attacks)
...
- 归属模块：`M15` → [15-winrm-lateral](/zh/modules/15-winrm-lateral)
