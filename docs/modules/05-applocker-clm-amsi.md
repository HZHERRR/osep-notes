...
> cheat sheet 依据：`Defense Evasion`（Enumerate Defenses / Disable AppLocker & CLM / Disable AMSI / Disable Defender）、`Utilities`（PowerView AV Evasion / Encryptors / Encode PowerShell Payloads）、`Payloads`（C# BIN Shellcode XOR Encrypted / Process Injection）
...
2. 准备编码/加密版 shellcode（XOR 最简单、最稳）：
   ...
   python3 scripts/python/m13-xor-encoder.py sc.bin --format c   # 输出 C 数组 + key
   ```
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/python/m13-xor-encoder.py` | 生成 XOR 编码后的 C 数组 | `sc.bin --format c` |
| `scripts/csharp/m01-shellcode-runner-x64.cs` | 自定义 C# Runner（含 XOR 解密） | 替换密文数组与 key |
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | PowerShell 路线时的 AMSI 处理 | 见脚本内说明 |


#### 源码 `scripts/python/m13-xor-encoder.py` {#scripts-python-m13-xor-encoder-py}
...
2. 若在**通信阶段**被终止 → 问题在 C2 通道而非注入（转 [`docs/09-c2-egress-channels.md`](/modules/09-c2-egress-channels)）。
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/csharp/m01-shellcode-runner-x64.cs` | 进程内执行（样本 A） | 换 payload 与 key |
| `scripts/powershell/m05-clm-bypass-runspace.ps1` | 托管内存执行备选 | 见脚本 |
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | 脚本路线的 AMSI 处理 | 见脚本 |


#### 源码 `scripts/powershell/m05-clm-bypass-runspace.ps1` {#scripts-powershell-m05-clm-bypass-runspace-ps1}
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/csharp/m05-installutil-runner.cs` | 托管程序集加载器模板 | 替换程序集路径/入口 |
| `scripts/powershell/m01-reflective-runner.ps1` | PowerShell 反射加载 | 替换 DLL 路径与参数 |


#### 源码 `scripts/csharp/m05-installutil-runner.cs` {#scripts-csharp-m05-installutil-runner-cs}
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/powershell/m05-applocker-enum.ps1` | 枚举有效规则 + 可写允许路径 | `-PayloadPath` 可选 |


#### 源码 `scripts/powershell/m05-applocker-enum.ps1` {#scripts-powershell-m05-applocker-enum-ps1}
...
3. 把 DLL 放到宿主的搜索路径，保持导出函数与调用约定与原 DLL 一致（见 [`docs/04-dll-sideloading.md`](/modules/04-dll-sideloading) 场景 12）。
4. 启动宿主，确认载荷执行且宿主功能正常。

**用到的脚本**：
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/c/m04-proxy-dll-sideload.c` | 全转发 Proxy DLL | 替换原 DLL 名 |
| `scripts/c/m04-proxy-dll-newadmin.c` | DLL 载荷（加管理员/反连） | LHOST/LPORT |
| `scripts/python/m04-build-sideload-package.py` | 打包宿主 + DLL | 宿主版本、架构 |


#### 源码 `scripts/c/m04-proxy-dll-sideload.c` {#scripts-c-m04-proxy-dll-sideload-c}
...
#### 源码 `scripts/c/m04-proxy-dll-newadmin.c` {#scripts-c-m04-proxy-dll-newadmin-c}
...
#### 源码 `scripts/python/m04-build-sideload-package.py` {#scripts-python-m04-build-sideload-package-py}
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/csharp/m05-workflow-compiler-runner.cs` | Workflow 宿主输入程序集 | 替换载荷逻辑 |
| `scripts/infra/m05-lolbas-notes.md` | 受信任宿主速查 | — |


#### 源码 `scripts/csharp/m05-workflow-compiler-runner.cs` {#scripts-csharp-m05-workflow-compiler-runner-cs}
...
#### 源码 `scripts/infra/m05-lolbas-notes.md` {#scripts-infra-m05-lolbas-notes-md}
...
- 策略枚举细节：`scripts/powershell/m05-applocker-enum.ps1`
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/infra/m05-xsl-exec.xsl` | XSL 脚本执行模板 | 替换 LHOST/URL |
| `scripts/infra/m05-lolbas-notes.md` | 宿主调用方式速查 | — |


#### 源码 `scripts/infra/m05-xsl-exec.xsl` {#scripts-infra-m05-xsl-exec-xsl}
...
| CLM 绕过 | 自定义 Runspace（`scripts/powershell/m05-clm-bypass-runspace.ps1`） |
| AMSI 处理 | `scripts/powershell/m05-amsi-bypass-variants.ps1` |

## 关联脚本清单

| 脚本 | 说明 |
|---|---|
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | AMSI 处理多个版本（按宿主选择） |
| `scripts/powershell/m05-clm-bypass-runspace.ps1` | 自定义 Runspace 绕过 CLM |
| `scripts/powershell/m05-applocker-enum.ps1` | 枚举有效规则与可写允许路径 |
| `scripts/csharp/m05-installutil-runner.cs` | InstallUtil 兼容程序集 |
| `scripts/csharp/m05-workflow-compiler-runner.cs` | Workflow 宿主输入程序集 |
| `scripts/infra/m05-xsl-exec.xsl` | XSL 脚本执行模板 |
| `scripts/infra/m05-lolbas-notes.md` | 受信任宿主速查 |
| `scripts/python/m13-xor-encoder.py` | shellcode 编码（场景 18 用） |
| `scripts/c/m04-proxy-dll-sideload.c` | Proxy DLL（场景 22 用） |
