...
| 文件 | 用途 |
|---|---|
| `scripts/c/m04-proxy-dll-sideload.c` | 纯转发 Proxy DLL（全导出转发 + 保持宿主正常），兼作场景 12 的“隔离对照版” |
| `scripts/c/m04-proxy-dll-newadmin.c` | 实弹版：加管理员 / 反连两种模式（编译期 `MODE` 宏切换），同样全导出转发 |
| `scripts/c/m04-proxy-dll-cpp.cpp` | C++ 版 + `.def` / 名字修饰 / 调用约定说明（含少量函数的手写转发示例） |
| `scripts/python/m04-build-sideload-package.py` | 把 宿主 + Proxy DLL + 改名原 DLL 打成 ZIP，校验 PE 架构与文件完整性 |
...
#### 源码 `scripts/c/m04-proxy-dll-cpp.cpp` {#scripts-c-m04-proxy-dll-cpp-cpp}
...
### 用到的脚本
- `scripts/c/m04-proxy-dll-newadmin.c`（实弹；编译命令见上）
- `scripts/python/m04-build-sideload-package.py`（打包 + 架构校验）
- `scripts/c/m04-proxy-dll-sideload.c`（排障时做“无载荷对照”，见场景 12）
...
### 用到的脚本
- `scripts/c/m04-proxy-dll-sideload.c`（隔离对照：无载荷纯转发）
- `scripts/c/m04-proxy-dll-newadmin.c`（实弹版，注意其 DllMain 只 `CreateThread`）
- `scripts/c/m04-proxy-dll-cpp.cpp`（.def / 调用约定注释 + 少量函数手写转发的写法示例）
...
# 打包（校验架构一致性后出 ZIP）
python3 scripts/python/m04-build-sideload-package.py --input-dir ./sideload \
