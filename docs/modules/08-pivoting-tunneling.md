...
> 配套脚本：`scripts/infra/m08-ligolo-ng-setup.sh`、`scripts/infra/m08-chisel-socks.sh`、`scripts/powershell/m08-port-forward.ps1`。
...
方案 B：Ligolo-ng（DEV 能执行 agent，Windows/Linux 均可）——见 `scripts/infra/m08-ligolo-ng-setup.sh`：
```bash
...
方案 C：chisel（DEV 能执行 chisel client）——见 `scripts/infra/m08-chisel-socks.sh`。
...
```
Windows 形态的 DEV 跳板也能用 `scripts/powershell/m08-port-forward.ps1` 的 portproxy 做等价转发。

### 用到的脚本
- `scripts/infra/m08-ligolo-ng-setup.sh`（proxy/agent/路由/reverse 子命令）
- `scripts/infra/m08-chisel-socks.sh`（备选转发 + proxychains）
- `scripts/powershell/m08-port-forward.ps1`（DEV 是 Windows 时的 netsh portproxy）
...
#### 源码 `scripts/infra/m08-ligolo-ng-setup.sh` {#scripts-infra-m08-ligolo-ng-setup-sh}
...
#### 源码 `scripts/infra/m08-chisel-socks.sh` {#scripts-infra-m08-chisel-socks-sh}
...
#### 源码 `scripts/powershell/m08-port-forward.ps1` {#scripts-powershell-m08-port-forward-ps1}
...
-- 或 xp_subdirs / xp_fileexist；低权限 SQL 也常能触发（见 [docs/11-mssql.md](/modules/11-mssql)）
```
...
### 用到的脚本
- `scripts/infra/m08-ligolo-ng-setup.sh`（reverse 子命令，反连转发的标准模板）
- `scripts/powershell/m08-port-forward.ps1`（Windows 跳板 portproxy，含回滚/清理）
- 触发侧模板见 [`docs/11-mssql.md`](/modules/11-mssql) 与 [`docs/16-ics-calendar.md`](/modules/16-ics-calendar)（认证触发手段）
...
- SMB/HTTP 中继要求目标与中继**同网段且不开 SMB 签名**（可先探测）；EPA/签名等条件见 [`docs/12-ad-attacks.md`](/modules/12-ad-attacks) ESC8 与 [`docs/16-ics-calendar.md`](/modules/16-ics-calendar)。
...
| 文档 | 内容 |
|---|---|
| [`docs/00-environment-and-infra.md`](/modules/00-environment-and-infra) | 端口规划（11601/1080/8081…）、日志纪律 |
| [`docs/11-mssql.md`](/modules/11-mssql) | 场景 35 的认证触发手段（xp_dirtree 等） |
| [`docs/16-ics-calendar.md`](/modules/16-ics-calendar) | 外部触发认证的另一种形态 |
| [`docs/09-c2-egress-channels.md`](/modules/09-c2-egress-channels) | 出网通道与"全阶段同路径"原则 |
| `scripts/infra/m08-ligolo-ng-setup.sh` 等 | 本模块脚本用法 |
