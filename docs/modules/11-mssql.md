...
> 覆盖场景：44、45
> cheat sheet 依据：`MSSQL`（Authentication / Enumeration / Relay Attacks / Linked Server / Privilege Escalation：Impersonation、Code Execution、CLR、Service Account to SYSTEM、MSSQLPwner）、`Credentials`（Relay Attacks / Cracking Hashes）
...
1. 用低权限登录连接实例，确认当前权限状态（先验证"确实不能执行系统命令"）：
   ...
   ```
   从 Windows（PowerShell + Invoke-SQLCmd 模板脚本亦可，见 `scripts/powershell/m11-powerupsql-templates.ps1`）。
...
6. 后续在 SQL 上真正取得执行权限的常见跳板（如果环境里其实还有 `sysadmin` 或可模拟对象）：
   ...
   ```
   详见 `scripts/powershell/m11-powerupsql-templates.ps1`（模拟、CLR 等模板）与场景 45（Linked Server 路线）。

**用到的脚本**：
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/linux/m11-responder-relay-sql.sh` | 一键起 Responder 捕获 / ntlmrelayx 中继 + 打印 SQL 触发命令 | `LHOST`、`LPORT`、`TARGET` |
| `scripts/powershell/m11-powerupsql-templates.ps1` | 实例发现、权限确认、审计、模拟/执行 | `-Instance TARGET` |
| `scripts/powershell/m11-sqlrecon-templates.ps1` | SQLRecon 对照模板（/m:info /m:xpcmd 等） | `/h:TARGET /a:WinToken` |
| `scripts/sql/m11-linked-server-queries.sql` | 需要跨实例枚举时的查询集（本场景可用于确认"另一个实例"） | 见文件头 |


#### 源码 `scripts/powershell/m11-powerupsql-templates.ps1` {#scripts-powershell-m11-powerupsql-templates-ps1}
...
#### 源码 `scripts/linux/m11-responder-relay-sql.sh` {#scripts-linux-m11-responder-relay-sql-sh}
...
#### 源码 `scripts/powershell/m11-sqlrecon-templates.ps1` {#scripts-powershell-m11-sqlrecon-templates-ps1}
...
#### 源码 `scripts/sql/m11-linked-server-queries.sql` {#scripts-sql-m11-linked-server-queries-sql}
...
- 准备好：`impacket-mssqlclient`（Linux）或 PowerUpSQL/SQLRecon（Windows）、`scripts/sql/m11-linked-server-queries.sql`。
...
7. **多跳（A→B→C）模板**——核心是"在 A 上对 B 发一条 EXEC，让 B 再对 C 发一条 EXEC"：
   ...
   - 完整模板见 `scripts/sql/m11-linked-server-queries.sql`（含每层引号标注）。
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/sql/m11-linked-server-queries.sql` | 链接枚举 / 单跳 / 多跳 / `EXEC ... AT` 全套模板（引号标注） | 替换 `TARGET2`/`TARGET3` 等 |
| `scripts/powershell/m11-powerupsql-templates.ps1` | `Get-SQLServerLinkCrawl`、`Invoke-SQLAudit`、开 xp_cmdshell 的 PS 侧封装 | `-Instance A_HOST` |
| `scripts/powershell/m11-sqlrecon-templates.ps1` | SQLRecon 的 link 模块对照（links / linkquery / linkcmd） | `/h:A_HOST /m:linkquery` |
| `scripts/linux/m11-responder-relay-sql.sh` | 若 B 是"中继目标"时的收尾路线 | `TARGET` |
...
| 文件 | 用途 |
|---|---|
| `scripts/sql/m11-linked-server-queries.sql` | Linked Server 枚举 / 单跳 / 多跳 / EXEC AT 模板与引号转义 |
| `scripts/powershell/m11-powerupsql-templates.ps1` | PowerUpSQL 发现 / 枚举 / 审计 / 模拟 / 执行 / CLR 模板 |
| `scripts/powershell/m11-sqlrecon-templates.ps1` | SQLRecon 模块对照（info / impersonate / xpcmd / olecmd / clr / links） |
| `scripts/linux/m11-responder-relay-sql.sh` | Responder / ntlmrelayx 中继一键流程 + SQL 触发命令打印 |
