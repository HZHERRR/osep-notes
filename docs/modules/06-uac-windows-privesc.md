...
> 技术路线对齐 `reference/osep-cheatsheet.md`（关键词：`UAC Bypass` `PrintSpoofer` `SigmaPotato` `FullPowers` `AlwaysInstallElevated` `Service Binary Hijacking`）。
> 相关模块：本模块只讲「拿到第一段立足点之后的本地提权」。横向移动见 [`docs/15-winrm-lateral.md`](/modules/15-winrm-lateral)，凭据抓取见 [`docs/07-credentials-lsass.md`](/modules/07-credentials-lsass)。
...
# 或准备无文件第二阶段：scripts/powershell/m06-fodhelper-uac.ps1 内嵌 PAYLOAD 变量
...
集成脚本：`scripts/powershell/m06-fodhelper-uac.ps1`（自动写入键、触发、可选延时清理）。

### 用到的脚本
- `scripts/powershell/m06-fodhelper-uac.ps1`
...
#### 源码 `scripts/powershell/m06-fodhelper-uac.ps1` {#scripts-powershell-m06-fodhelper-uac-ps1}
...
### 用到的脚本
- `scripts/powershell/m06-sigmapotato-reflect.ps1`（离线自包含版本——不落地 EXE，用反射方式执行 SigmaPotato 的核心逻辑；同时是“自动工具失败时的手工备选”的载体）
...
> SpoolSample（打印假脱机）在此场景的用法：它是**诱导认证**而非直接提权——让 `potato`/`printbug` 触发 SYSTEM 对被控机的认证，配合中继或 RPC 利用（典型是 Printerbug → Relay 到 LDAP/ADCS，见 [`docs/12-ad-attacks.md`](/modules/12-ad-attacks) ESC8）。如果本机土豆路线全失败，这是考题的“备选路径”。
...
#### 源码 `scripts/powershell/m06-sigmapotato-reflect.ps1` {#scripts-powershell-m06-sigmapotato-reflect-ps1}
...
### 准备（攻击机侧）
```bash
# 用 scripts/c/m06-service-binary-payload.c 编译 payload：
...
### 用到的脚本
- `scripts/c/m06-service-binary-payload.c`（服务 payload：启动后派生反连 shell 或加管理员，且可选地替身保持服务“活着”）
- `scripts/powershell/m06-service-hijack.ps1`（保存/恢复原配置的完整劫持+回滚自动化）
...
#### 源码 `scripts/c/m06-service-binary-payload.c` {#scripts-c-m06-service-binary-payload-c}
...
提权后固定动作：`whoami /groups` 记录新完整性 → 如需凭据抓取见 [`docs/07-credentials-lsass.md`](/modules/07-credentials-lsass) → 横向见 [`docs/15-winrm-lateral.md`](/modules/15-winrm-lateral)。
