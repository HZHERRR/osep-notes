...
> 技术对齐：`reference/osep-cheatsheet.md` 的 `Payloads (XOR Payload Encoder / Simple Loader / Shared Library LD PRELOAD / Shared Library LD LIBRARY Path)`、`Abusing SUIDs`、`SSH Hijacking with ControlMaster / SSH Agent Forwarding`、`Artifactory (JFrog)` 各节。
...
### 用到的脚本
- `scripts/python/m13-xor-encoder.py`：XOR 编码 stage2（文件模式 / C 数组模式）。
- `scripts/c/m13-simple-loader.c`：读取/内嵌编码 payload，内存解码执行；含业务输出模式与心跳生命周期逻辑。
...
#### 源码 `scripts/c/m13-simple-loader.c` {#scripts-c-m13-simple-loader-c}
 ...
 * 配套编码脚本（scripts/python/m13-xor-encoder.py，攻击机侧）：
...
### 用到的脚本
- `scripts/python/m13-xor-encoder.py`（改变磁盘签名面）。
- `scripts/c/m13-simple-loader.c`（内存解码，避免明文落盘）。
...
### 用到的脚本
- `scripts/c/m13-shared-library-ldpreload.c`：LD_PRELOAD 注入（不依赖缺失库）。
- `scripts/c/m13-shared-library-ldlibrarypath.c`：同名替换缺失库（必须核对库名与符号导出）。
...
#### 源码 `scripts/c/m13-shared-library-ldpreload.c` {#scripts-c-m13-shared-library-ldpreload-c}
...
#### 源码 `scripts/c/m13-shared-library-ldlibrarypath.c` {#scripts-c-m13-shared-library-ldlibrarypath-c}
...
### 用到的脚本
- `scripts/linux/m13-sudo-gtfobins.sh`：解析 `sudo -l` 放行条目，给出并执行对应的 vim/find/lua 逃逸；带参数限制时的读取/外带模式。
...
#### 源码 `scripts/linux/m13-sudo-gtfobins.sh` {#scripts-linux-m13-sudo-gtfobins-sh}
...
### 用到的脚本
- `scripts/linux/m13-artifactory-replace.sh`：制品定位、备份、替换、权限修复、校验和核对、恢复。
...
#### 源码 `scripts/linux/m13-artifactory-replace.sh` {#scripts-linux-m13-artifactory-replace-sh}
...
#       python3 + scripts/python/m13-xor-encoder.py（把载荷编码后内嵌进替换制品）
# 使用：
...
### 用到的脚本
- `scripts/linux/m13-ssh-controlmaster-hijack.sh`：自动发现 ControlMaster socket 与可用 agent socket，逐一尝试复用登入，输出成功的目标与身份。
...
#### 源码 `scripts/linux/m13-ssh-controlmaster-hijack.sh` {#scripts-linux-m13-ssh-controlmaster-hijack-sh}
...
| 脚本 | 场景 | 用途 |
|---|---|---|
| `scripts/python/m13-xor-encoder.py` | 36/37 | XOR 编码/解码 payload（文件或 C 数组） |
| `scripts/c/m13-simple-loader.c` | 36/37 | 内存解码执行 stage2；业务输出/心跳模式 |
| `scripts/c/m13-shared-library-ldpreload.c` | 38 | LD_PRELOAD 注入 .so |
| `scripts/c/m13-shared-library-ldlibrarypath.c` | 38 | 同名替换缺失库（LD_LIBRARY_PATH） |
| `scripts/linux/m13-sudo-gtfobins.sh` | 39 | sudo 放行单程序的 GTFOBins 逃逸 |
| `scripts/linux/m13-artifactory-replace.sh` | 40 | 制品备份/替换/恢复 |
| `scripts/linux/m13-ssh-controlmaster-hijack.sh` | 48 | ControlMaster/agent 复用连接发现与借用 |
