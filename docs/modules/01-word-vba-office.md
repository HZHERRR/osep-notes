...
> 覆盖场景：1、2、3、4、5
> cheat sheet 依据：`Phishing` → `Microsoft Word`（Callback Pinging / Determine Target Architecture / Shellcode Runner - Meterpreter Encrypted / Shellcode Runner - C# VBA Encrypted / PowerShell Stager - Encrypted & Obfuscated / PowerShell Stager - Unencrypted / Process Injection / Process Hollow）、`Payloads`（C# BIN Shellcode XOR Encrypted / Process Injection、PowerShell Reflective One-Liners）、`Upgrading Shells`
> 教材依据：第 4 章（宏与初始访问）、第 10 章（进程迁移与生命周期）、第 11 章（免杀与编码）
> 前置依赖：可投递的 `.docm`/`.doc`；目标安装 Office；攻击机有 HTTP 投递服务（`scripts/python/m00-delivery-server.py`）；x86/x64 两套 Runner 已编译
...
1. 起投递服务并盯日志：
   ```bash
   python3 scripts/python/m00-delivery-server.py --port 80 --dir ~/osep/payloads
   ```
2. 准备三份材料（全部事先验证过）：
   | 材料 | 文件 | 作用 |
   |---|---|---|
   | 无害回调宏 | `scripts/vba/m01-callback-ping.vba` | 只证明宏触发与出网 |
   | 架构识别宏 | `scripts/vba/m01-detect-arch.vba` | 回传 Office 位数 |
   | Runner | `scripts/vba/m01-shellcode-runner-vba-x86.vba` / `-x64.vba` | 按位数选择 |
3. 生成两套 shellcode（x86/x64 分开）：
   ...
   ```
4. 起监听：`bash scripts/infra/m00-listener.sh 4444`
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/vba/m01-callback-ping.vba` | 无害回调，验证宏与出网 | 替换 `LHOST` |
| `scripts/vba/m01-detect-arch.vba` | WMI 判断 Office 位数并回传 | 替换 `LHOST` |
| `scripts/vba/m01-shellcode-runner-vba-x86.vba` | 32 位 Runner | 替换 shellcode 数组 |
| `scripts/vba/m01-shellcode-runner-vba-x64.vba` | 64 位 Runner | 替换 shellcode 数组 |
| `scripts/vba/m01-shellcode-runner-vba-archbranch.vba` | 自动判断位数并分支 | 同时内嵌两套 shellcode |


#### 源码 `scripts/python/m00-delivery-server.py` {#scripts-python-m00-delivery-server-py}
...
依赖：Python 3.7+（标准库）；HTTPS 需要 cert/key（可用 openssl 或 scripts/infra/m00-build-payloads.sh 生成）
...
#### 源码 `scripts/vba/m01-callback-ping.vba` {#scripts-vba-m01-callback-ping-vba}
...
'          (see scripts/infra/m01-http-file-server.md).
' Usage:
'   1. Start a listener that records HTTP hits, e.g. on Kali:
'        sudo python3 scripts/infra/m01-serve.py /var/www/html 80    (or nginx, see infra doc)
...
#### 源码 `scripts/vba/m01-detect-arch.vba` {#scripts-vba-m01-detect-arch-vba}
...
#### 源码 `scripts/vba/m01-shellcode-runner-vba-x86.vba` {#scripts-vba-m01-shellcode-runner-vba-x86-vba}
...
Private Function GetEncodedShellcode() As Byte()
    ' ← 用 scripts/python/m13-xor-encoder.py --format vba 生成后替换
    Dim tmp(3) As Byte
...
#### 源码 `scripts/vba/m01-shellcode-runner-vba-x64.vba` {#scripts-vba-m01-shellcode-runner-vba-x64-vba}
...
Private Function GetEncodedShellcode() As Byte()
    ' ← 用 scripts/python/m13-xor-encoder.py --format vba 生成后替换本函数
    ' 示例（占位）：
    Dim tmp(3) As Byte
    tmp(0) = &H1B: tmp(1) = &H1B: tmp(2) = &H1B: tmp(3) = &H1B
    GetEncodedShellcode = tmp
...
#### 源码 `scripts/infra/m00-listener.sh` {#scripts-infra-m00-listener-sh}
...
echo "[*] 提示：拿到会话先跑 whoami /priv、systeminfo、出网探测（见 scripts/powershell/m00-recon-defenses.ps1）"
...
#### 源码 `scripts/vba/m01-shellcode-runner-vba-archbranch.vba` {#scripts-vba-m01-shellcode-runner-vba-archbranch-vba}
...
```text
编译/准备：scripts/vba/m01-shellcode-runner-vba-x64.vba（含 PtrSafe 声明与位数分支）
备用：scripts/vba/m01-embedded-dotnet-runner.vba（VBA 内加载 .NET 程序集）
```
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/vba/m01-shellcode-runner-vba-x64.vba` | 进程内执行，不创建子进程 | shellcode 数组 |
| `scripts/vba/m01-embedded-dotnet-runner.vba` | VBA 内加载 .NET 程序集 | 程序集 Base64 |
| `scripts/vba/m01-callback-ping.vba` | 排除"宏整体被拦" | LHOST |


#### 源码 `scripts/vba/m01-embedded-dotnet-runner.vba` {#scripts-vba-m01-embedded-dotnet-runner-vba}
...
1. 短宏第一阶段（尽量短，减少宏自身特征）：
   ...
   End Sub
   ```
2. 与 **PowerShell 宿主匹配**的 AMSI 处理脚本：`scripts/powershell/m05-amsi-bypass-variants.ps1`
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | AMSI 处理多版本 + 探针 | `-Variant 1..6` |
| `scripts/powershell/m01-stager-download-encrypted.ps1` | 加密/混淆的下载执行 stage2 | LHOST/URL |
| `scripts/powershell/m01-reflective-runner.ps1` | 反射加载，绕过对落地脚本的扫描 | 程序集路径 |


#### 源码 `scripts/powershell/m05-amsi-bypass-variants.ps1` {#scripts-powershell-m05-amsi-bypass-variants-ps1}
...
#### 源码 `scripts/powershell/m01-stager-download-encrypted.ps1` {#scripts-powershell-m01-stager-download-encrypted-ps1}
...
说明：
  - 攻击机侧先编码：python3 scripts/python/m13-xor-encoder.py stage2.ps1 --format b64 --key 42
...
#### 源码 `scripts/powershell/m01-reflective-runner.ps1` {#scripts-powershell-m01-reflective-runner-ps1}
...
3. **stage2 下载被拦但脚本没被拦** → 是网络/代理问题，转 [`docs/09-c2-egress-channels.md`](/modules/09-c2-egress-channels)。
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/powershell/m01-reflective-runner.ps1` | 反射加载 .NET 程序集 | `-Path` / `-Base64` |
| `scripts/csharp/m01-shellcode-runner-x64.cs` | 预编译 Runner 源码 | shellcode 密文 + key |
| `scripts/powershell/m01-stager-download-encrypted.ps1` | 加密 stager | LHOST/URL |


#### 源码 `scripts/csharp/m01-shellcode-runner-x64.cs` {#scripts-csharp-m01-shellcode-runner-x64-cs}
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/vba/m01-embedded-dotnet-runner.vba` | 内嵌执行，减少依赖 | 程序集 Base64 |
| `scripts/powershell/m01-reflective-runner.ps1` | 迁移后重新加载载荷 | 程序集/字节 |
| `scripts/powershell/m06-service-hijack.ps1` | 有管理员时的服务级持久化 | 服务名 |


#### 源码 `scripts/powershell/m06-service-hijack.ps1` {#scripts-powershell-m06-service-hijack-ps1}
...
| 脚本 | 说明 |
|---|---|
| `scripts/vba/m01-callback-ping.vba` | 无害回调宏 |
| `scripts/vba/m01-detect-arch.vba` | 位数识别并回传 |
| `scripts/vba/m01-shellcode-runner-vba-x86.vba` / `-x64.vba` | 进程内 Runner |
| `scripts/vba/m01-shellcode-runner-vba-archbranch.vba` | 位数分支 Runner |
| `scripts/vba/m01-embedded-dotnet-runner.vba` | 内嵌 .NET 加载（无出网也可用） |
| `scripts/powershell/m01-stager-download-encrypted.ps1` | 加密下载执行 |
| `scripts/powershell/m01-reflective-runner.ps1` | 反射加载 |
| `scripts/csharp/m01-shellcode-runner-x64.cs` | 预编译 Runner 源码 |
| `scripts/powershell/m05-amsi-bypass-variants.ps1` | AMSI 处理（跨模块共用） |
