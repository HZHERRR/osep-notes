...
> 覆盖场景：9–10
> cheat sheet 依据：`JScript`（`Simple Meterpreter Dropper` / `Meterpreter Loader with DotNetToJScript` / `Meterpreter with SuperSharpShooter`）  |  教材依据：第 7、9 章（场景 9）、§12.6（场景 10）
...
2. 把第 1 步输出粘进 `scripts/csharp/m03-dotnettojscript-payload.cs` 的 `PAYLOAD` 字节数组，在工具机编译：
   ```bat
...
**执行步骤**：
1. 确认目标脚本入口与宿主位数（可用 `scripts/jscript/m03-wsh-amsi-probe.js` 的宿主信息段）：
   ```bat
...
5. 若脚本入口是"下载后执行"而目标**能**跑脚本但不能跑 EXE：`scripts/jscript/m03-simple-dropper.js` 负责把第二阶段**脚本/数据文件**拉到目标（`RUN_AFTER_DOWNLOAD=0`），再用受信任宿主执行；不要让它直接 `Run(pay.exe)`。

**用到的脚本**：
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/csharp/m03-dotnettojscript-payload.cs` | C# 第二阶段（[ComVisible] 类，构造时注入 shellcode） | `PAYLOAD`（msfvenom csharp 字节）、`TARGET_PROCESS` |
| `scripts/jscript/m03-dotnettojscript-loader.js` | DotNetToJScript 产物(runner.js)的骨架/粘贴容器 + 预检 | 生成命令见文件头 |
| `scripts/jscript/m03-supersharpshooter-loader.js` | SuperSharpShooter 产物的分离式加载（读 `payload.js` 再执行） | `STAGE2_JS` 路径 |
| `scripts/jscript/m03-simple-dropper.js` | 纯下载/保存（脚本入口场景的传输层） | `URL`、`DROP_PATH`、`RUN_AFTER_DOWNLOAD` |


#### 源码 `scripts/csharp/m03-dotnettojscript-payload.cs` {#scripts-csharp-m03-dotnettojscript-payload-cs}
...
#### 源码 `scripts/jscript/m03-wsh-amsi-probe.js` {#scripts-jscript-m03-wsh-amsi-probe-js}
...
#### 源码 `scripts/jscript/m03-simple-dropper.js` {#scripts-jscript-m03-simple-dropper-js}
...
#### 源码 `scripts/jscript/m03-supersharpshooter-loader.js` {#scripts-jscript-m03-supersharpshooter-loader-js}
...
**准备（攻击机侧）**：
1. 准备实验脚本 `scripts/jscript/m03-wsh-amsi-probe.js`，先在本机与靶机各跑一遍，记录宿主位数、.NET 目录、动态执行段的 PASS/BLOCKED 结果：
   ```bat
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `scripts/jscript/m03-wsh-amsi-probe.js` | 报告宿主信息 + 对动态执行内容做 PASS/BLOCKED 实验 | 无参数；`EVAL_TEST_CONTENT` 可改 |
| `scripts/jscript/m03-simple-dropper.js` | 下载阶段 B 并换新 cscript 进程执行（A/B 分离） | `URL`、`DROP_PATH`、`RUN_AFTER_DOWNLOAD` |
| `scripts/jscript/m03-supersharpshooter-loader.js` | 分离式第二阶段加载实验 | `STAGE2_JS` |
| `scripts/jscript/m03-dotnettojscript-loader.js`、`scripts/csharp/m03-dotnettojscript-payload.cs` | 桥接产物容器与 C# 第二阶段 | 见场景 9 |
...
| 脚本 | 行数约 | 说明 |
|---|---|---|
| `scripts/jscript/m03-simple-dropper.js` | 60 | 下载/保存/可选执行（传输层，A/B 分离用） |
| `scripts/jscript/m03-dotnettojscript-loader.js` | 90 | DotNetToJScript 产物骨架/粘贴容器 + 预检 |
| `scripts/jscript/m03-supersharpshooter-loader.js` | 90 | SuperSharpShooter 产物分离式加载（实验） |
| `scripts/jscript/m03-wsh-amsi-probe.js` | 100 | WSH 宿主信息 + AMSI 动态内容实验 |
| `scripts/csharp/m03-dotnettojscript-payload.cs` | 110 | C# 第二阶段（[ComVisible] 注入类） |
